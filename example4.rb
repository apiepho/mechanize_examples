...
class Team
	...
	def initialize(browser, options, href)
		...
		@games = []
		json_decoded.each do |game_json|
			next if not options.games.nil? and @games.length >= options.games.to_i
			@games << Game.new(browser, options, game_json) if not game_json.nil? and not game_json["game_id"].nil?
		end        
