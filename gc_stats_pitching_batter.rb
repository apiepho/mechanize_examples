...
class StatsPitchingBatterResults

    def initialize(fteam, team_id, fname, linitial, player_id)
        uri = GC_PLAYER_PITCHING_BATTER_URI % [GC_BASE_URI, fteam, team_id, fname, linitial, player_id]
        $browser.goto(uri)
        temp = $browser.html
    ...
    def display
        puts "%s%s" % [ $indent.str, "stats_pitching_batter: "]
        $indent.increase
